    function [ X_dmd ] = compute_dmdMr_reconstruction_pot2_4(tree,dt,L)
 ...
 %---------------------------------------     
      ...
      for ii=1:L    %Andar
        ...
        for jj=1:np    %Partição
          ...
          si = (jj-1)*sp + 1;
          sf =  jj*sp;
%          dtf = dt * tree{ii,jj}.sub;
          for nn=1:length(omega)  %Modo
            
            Mode = repmat(modes(:,nn),1,sp);
            
              X_dmd(:,si:sf)= X_dmd(:,si:sf) + ... 
              Mode*b(nn).*exp(omega(nn)*T(:,si:sf));   %/dt
